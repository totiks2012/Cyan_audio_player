...
func (m *model) Init() tea.Cmd {
	if m.state.CurrentIndex >= 0 && m.state.CurrentIndex < len(m.state.Playlist) {
		_ = m.player.Start(m.state.Playlist[m.state.CurrentIndex], m.state.Volume)
	}
	...
	case tea.KeyMsg:
		...
		case "<", ",":
			...
			for i, item := range m.fmItems {
				if item.name == oldDir { m.fmCur = i; break }
			}
		case " ": _, _ = m.player.SendCommand([]interface{}{"cycle", "pause"})
		case "left": _, _ = m.player.SendCommand([]interface{}{"seek", -5})
		case "right": _, _ = m.player.SendCommand([]interface{}{"seek", 5})
		}
		m.sync()
...
func (m *model) changeVolume(delta int) {
	...
	if m.state.Volume < 0 { m.state.Volume = 0 }
	if m.state.Volume > 100 { m.state.Volume = 100 }
...
func (m *model) action() {
	if m.focus == 0 {
		if len(m.fmItems) > 0 && m.fmCur < len(m.fmItems) {
			...
			if it.isDir { m.state.Cwd = it.path; m.fmCur, m.fmOff = 0, 0; m.refresh() } else { m.add() }
		}
	} else {
		if len(m.plItems) > 0 && m.plCur < len(m.plItems) {
			m.state.CurrentIndex = m.plCur
			_ = m.player.Start(m.state.Playlist[m.plCur], m.state.Volume)
		}
	}
}
...
func (m *model) add() {
	...
	if it.isDir {
		...
		for _, f := range files {
			if !f.IsDir() { m.state.Playlist = append(m.state.Playlist, filepath.Join(it.path, f.Name())) }
		}
	} else { m.state.Playlist = append(m.state.Playlist, it.path) }
	m.refresh(); m.save()
}
...
func (m *model) refresh() {
	...
	sort.Slice(m.fmItems, func(i, j int) bool {
		if m.fmItems[i].isDir != m.fmItems[j].isDir { return m.fmItems[i].isDir }
		return strings.ToLower(m.fmItems[i].name) < strings.ToLower(m.fmItems[j].name)
	})
	m.plItems = nil
	for _, p := range m.state.Playlist { m.plItems = append(m.plItems, displayItem{p, filepath.Base(p), false}) }
	m.sync()
}

func (m *model) doSearch() {
	if m.searchInput == "" { m.refresh(); return }
	m.refresh()
	var filtered []displayItem
	items := m.fmItems; if m.focus == 1 { items = m.plItems }
	searchLower := strings.ToLower(m.searchInput)
	for _, it := range items {
		if strings.Contains(strings.ToLower(it.name), searchLower) { filtered = append(filtered, it) }
	}
	if m.focus == 0 { m.fmItems = filtered } else { m.plItems = filtered }
	m.fmCur, m.plCur = 0, 0
	m.sync()
}
